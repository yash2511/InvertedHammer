...
Usage:
    ...
    python main.py --all                # Run NSE (Hammer + Shooting Star) + Forex
...
def job_nse_combined():
    """Run both NSE patterns and send ONE combined Telegram message."""
    logger.info("=" * 50)
    logger.info("NSE Combined Scan (Inverted Hammer + Shooting Star)")
    logger.info("=" * 50)
    hammer_matches = nse_scan()
    star_matches = ss_scan()
    send_combined_alert(hammer_matches, star_matches)
    logger.info("NSE Combined: %d Inverted Hammer, %d Shooting Star",
                len(hammer_matches), len(star_matches))
...
def main():
    ...
    logger.info("Inverted Hammer + Shooting Star: %s daily (Mon-Fri)", SCAN_TIME)
    logger.info("Waiting...\n")

    schedule.every().monday.at(SCAN_TIME).do(job_nse_all)
    schedule.every().tuesday.at(SCAN_TIME).do(job_nse_all)
    schedule.every().wednesday.at(SCAN_TIME).do(job_nse_all)
    schedule.every().thursday.at(SCAN_TIME).do(job_nse_all)
    schedule.every().friday.at(SCAN_TIME).do(job_nse_all)
